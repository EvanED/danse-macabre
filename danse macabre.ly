...
violin = \relative c' {
  ...
  r4 r f'^"pizz"^\markup{\italic "marcato"} |
  e r f |
  d r e |
  cs r d |
  ...
  \tuplet 3/2 4 {b\cresc( as a gs g fs f e es } |
  ...
  \repeat unfold 4 {
    R1*3/4 |
    <<
      { d2\ff d4 } % TODO: only first repeat
      \\
      { d2 d4 }
    >>
  }
  ...
  \bar "||"
  R1*3/4*1 |
  ef''4-.\f_"(du talon.)"^"ad lib." r8-. bf'-. bf4-. |
  
  % PAGE 5 LINE 10
  \break
  ef,4-. g2->_\markup{\italic "dim."}^\markup{\italic "long"}\fermata |
  R1*3/4*10
  r1*3/4\fermata |
